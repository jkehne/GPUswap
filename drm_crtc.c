...
void drm_connector_cleanup(struct drm_connector *connector)
{
	...
	list_for_each_entry_safe(mode, t, struct drm_display_mode, &connector->user_modes, head)
		drm_mode_remove(connector, mode);

	/* OSOL_drm: kfree(connector->fb_helper_private, sizeof(struct drm_fb_helper_cmdline_mode)); */
	mutex_lock(&dev->mode_config.mutex);
...
int drm_mode_getresources(DRM_IOCTL_ARGS)
{
	...
	if (card_res->count_fbs >= fb_count) {
		...
		list_for_each_entry(fb, struct drm_framebuffer, &file_priv->fbs, head) {
...
int drm_mode_setcrtc(DRM_IOCTL_ARGS)
{
	...
	if (crtc_req->mode_valid) {
		...
		if (crtc_req->fb_id == -1) {
			list_for_each_entry(crtcfb, struct drm_crtc, 
...
int drm_mode_addfb(DRM_IOCTL_ARGS)
{
	...
	r->fb_id = fb->base.id;
	list_add(&fb->filp_head, &file_priv->fbs, (caddr_t)fb);
...
int drm_mode_rmfb(DRM_IOCTL_ARGS)
{
	...
	list_for_each_entry(fbl, struct drm_framebuffer, &file_priv->fbs, filp_head)
		if (fb == fbl)
			found = 1;
			       ...
			       struct drm_display_mode *mode)
{
	...
	list_for_each_entry_safe(match_mode, t, struct drm_display_mode, &connector->user_modes, head) {
 ...
 * Zero on success, errno on failure.
 */
int drm_mode_attachmode_ioctl(DRM_IOCTL_ARGS)
{
 ...
 * Zero on success, errno on failure.
 */
int drm_mode_detachmode_ioctl(DRM_IOCTL_ARGS)
{
	struct drm_mode_object *obj;
	struct drm_mode_mode_cmd *mode_cmd = data;
	struct drm_connector *connector;
			       ...
			       struct drm_property_blob *blob)
{
	...
	list_del(&blob->head);
	kfree(blob, sizeof(struct drm_property_blob));
}
