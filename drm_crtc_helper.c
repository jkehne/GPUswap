				 ...
				 bool *enabled, int width, int height)
{
	...
	list_for_each_entry(connector, struct drm_connector, &dev->mode_config.connector_list, head) {
		...
		if (!modes[i] && !list_empty(&connector->modes)) {
			list_for_each_entry(modes[i], struct drm_display_mode, &connector->modes, head)
				{ if (1) break; }
		}
...
out:
	kfree(crtcs, dev->mode_config.num_connector * sizeof(struct drm_crtc *));
	return best_score;
}
...
static void drm_setup_crtcs(struct drm_device *dev)
{
	...
	kfree(crtcs, dev->mode_config.num_connector * sizeof(struct drm_crtc *));
	kfree(modes, dev->mode_config.num_connector * sizeof(struct drm_display_mode *));
	kfree(enabled, dev->mode_config.num_connector * sizeof(bool));
}
...
bool drm_helper_plugged_event(struct drm_device *dev)
{
	...
	/* OSOL_drm: dev->mode_config.funcs->fb_changed(dev); */
...
bool drm_helper_initial_config(struct drm_device *dev)
{
	...
	/* OSOL_drm: drm_fb_helper_parse_command_line(dev); */
